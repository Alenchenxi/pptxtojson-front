...
function shapeGear(h, points) {
  ...
  for (; a <= pi2 + angle; a += angle) {
    if (toggle) {
      d +=
        ' L' + (cx + radiusI * Math.cos(a - taperAI)) + ',' + (cy + radiusI * Math.sin(a - taperAI))
      d +=
        ' L' + (cx + radiusO * Math.cos(a + taperAO)) + ',' + (cy + radiusO * Math.sin(a + taperAO))
    } else {
      d +=
        ' L' + (cx + radiusO * Math.cos(a - taperAO)) + ',' + (cy + radiusO * Math.sin(a - taperAO))
      d +=
    ...
    case 'halfFrame':
      {
        ...
        if (shapAdjst_ary) {
          for (const adj of shapAdjst_ary) {
            ...
            if (sAdj_name === 'adj1') {
              sAdj1_val =
                parseInt(getTextByPathList(adj, ['attrs', 'fmla']).substring(4)) * RATIO_EMUs_Points
            } else if (sAdj_name === 'adj2') {
              sAdj2_val =
                parseInt(getTextByPathList(adj, ['attrs', 'fmla']).substring(4)) * RATIO_EMUs_Points
            }
    ...
    case 'rightArrow':
      {
        ...
        if (shapAdjst_ary) {
          ...
          for (const adj of shapAdjst_ary) {
            ...
            if (sAdj_name === 'adj1') {
              sAdj1_val =
                0.5 - parseInt(getTextByPathList(adj, ['attrs', 'fmla']).substring(4)) / 200000
            } else if (sAdj_name === 'adj2') {
              const sAdj2_val2 =
                parseInt(getTextByPathList(adj, ['attrs', 'fmla']).substring(4)) / 100000
    ...
    case 'flowChartMagneticDrum':
      {
        ...
        if (shapType === 'flowChartMagneticDrum') {
          dVal = dVal
            .replace(/([MLQC])\s*([-\d.e]+)\s*([-\d.e]+)/gi, (match, command, x, y) => {
        ...
        if (shapAdjst_ary) {
          for (const adj of shapAdjst_ary) {
            ...
            if (sAdj_name === 'adj1') {
              ...
              adj2 =
                ((parseInt(getTextByPathList(adj, ['attrs', 'fmla']).substring(4)) / 60000) *
                  Math.PI) /
                180
            } else if (sAdj_name === 'adj3') {
              adj3 =
